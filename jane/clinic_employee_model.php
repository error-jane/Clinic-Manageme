...
class Clinic_Employee_Conn extends DMC
{
    public function insertEmployee($obj)
    {
        ...
        (`employeeID`, `firstname`, `lastname`, `date_of_birth`, `phone_number`, `address`, `roleID`, `email`, `gender`, `salary`, `pictureID`)
        VALUES 
        ('01003', '$firstname', '$lastname', '$date_of_birth', '$phone_number', '$address', '$roleID', '$email', '$gender', '$salary', '02')";
        $this->insert($sql);
    }
    ...
    public function getEmployeeSearch($msg)
    {
        ...
        WHERE employee.firstname LIKE '$msg%' OR role.rolename LIKE '$msg%' OR employee.employeeID LIKE '$msg%'";
        $objEmployee = $this->select($sql);
        return $objEmployee;
    }   
